...
- (void)viewDidLoad {
    ...
    NSString *htmlPath = [[NSBundle mainBundle] pathForResource:@"wshTest" ofType:@"html"];
    ...
    [self.bridge registerHandler:@"getBlogNameFromObjC" handler:^(id data, WVJBResponseCallback responseCallback) {
        ...
        if (responseCallback) {
            // 反馈给JS
            responseCallback(@{@"blogName": @"wsh7365062的博客"});
        }
        // 一直常见的跳转
    ...
    [self.bridge callHandler:@"getUserInfos" data:@{@"name": @"wsh7365062"} responseCallback:^(id responseData) {
